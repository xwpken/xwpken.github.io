...
\left[\begin{matrix}
\pmb{r}_{E}\\
\pmb{f}_{F}\end{matrix}\right]$

where E-nodes for displacements known (essential), F-nodes for displacements unknown (free)

Solve $\pmb{d}_{F}$ 
from second row then taken into first row to get 
$\pmb{r}_{E}$
+ Draw 0 and set 1

$\begin{bmatrix}1 & 0 & 0  \\\\0 & {K}_{22} & {K}_{23}  \\\\0 & {K}_{32} & {K}_{33} \end{bmatrix}\begin{bmatrix}u_{1}\\\\u_{2}\\\\u_{3}\end{matrix}\right]=\begin{bmatrix}\overline{u}_1\\\\f_{2}-\overline{u}_1K_{21}\\\\f_3-\overline{u}_1K_{31}\end{bmatrix}$
